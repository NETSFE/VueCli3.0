<template>
  <div class="login-mask" v-show="needLogin">
    <div class="login-box" v-if="!needCheck">
      <p class="login-tit">送你一份来自民国的穿越礼！</p>
      <span class="login-box-text">输入手机号码，查看<br/>惊喜！</span>
      <div class="login-box-form">
        ...
        <div class="login-field btn-field">
          <button v-on:click="login" v-bind:class="{active: loginActive}" class="login-btn"
            data-event-category="财女-测颜值-领取一重礼" data-event-action="click" data-event-param="财女-测颜值-领取一重礼" otype="button" otitle="财女-测颜值-领取一重礼"
          >立即领取</button>
        </div>
      </div>
      <span class="login-box-text1">平安财女季，投资即<br/>送iPhoneX，3000万<br/>红包任性领！</span>
      <div class="login-field btn-field login-btn2">
        <button v-on:click="jumpto" v-bind:class="{active: loginActive}" class="login-btn login-btn3"
          data-event-category="财女-测颜值-领取二重礼" data-event-action="click" data-event-param="财女-测颜值-领取二重礼" otype="button" otitle="财女-测颜值-领取二重礼"
        >立即领取</button>
      </div>
    </div>
    ...
    <div class="login-box-check" v-if="needCheck">
      <div v-on:click.stop="needCheck=false" class="close-btn"></div>
      <div class="check-title">
        <p class="login-text">为了您的账号安全，请进行<br/>身份验证！</p>
      </div>
      <div class="check-div">
      ...
      <div class="check-btn-field">
        <button v-on:click.stop="checkLoginImage" v-bind:class="{active: checkActive}" class="login-btn4">确认</button>
      </div>
    </div>
  </div>
</template>
<script>
  ...
  export default {
    ...
    methods: {
      ...
      jumpto: function() {
        window.location = '//m.pingan.com/c3/huodong/cnj.html'
      },
      clearTip: function() {
        this.loginTip = ''
      },
...
<style lang="css" scoped>
  * {
  ...
  .login-mask {
    ...
    height: 667px;
    overflow: auto;
    background-image: url(../../assets/cyhkj/bg.jpg);
    background-size: 100% 100%;
  }
  .login-box {
    width: 365px;
    height: 575px;
    background-image: url(../../assets/cyhkj/loginBackground.png);
    background-size: 100% 100%;
    margin: 20px auto 0;
  }
  .login-box-form {
  ...
  .login-btn {
    ...
    height: 100%;
    background: transparent;
    background-image: url(../../assets/cyhkj/indexBtn.png);
    background-size: 100% 100%;
    font-size: 16px;
    color: #FEFEFE;
    outline: none;
  ...
  .login-box-check {
    ...
    height: 400px;
    margin: 20px auto;
    background-image: url(../../assets/cyhkj/loginCheck.png);
    background-size: 100% 100%;
    overflow: hidden;
  }
  .check-title {
  ...
  .close-btn {
    ...
    width: 29px;
    height: 29px;
    background: url("../../assets/cyhkj/close.png") no-repeat center center;
    background-size: 100% 100%;
  }
  .loading {
    position: fixed;
